...
for y in range(0, len(grid)):
	for x in range(0, len(grid[y])):
		#horizontal
		if x+3 < len(grid[y]):
			subProduct = grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3]
			if subProduct > biggestProduct:
				biggestProduct = subProduct

		#vertical
		if y+3 < len(grid):
			subProduct = grid[y][x] * grid[y+1][x] * grid[y+2][x] * grid[y+3][x]
			if subProduct > biggestProduct:
				biggestProduct = subProduct

		#diagonal left
		if y+3 < len(grid) and x+3 < len(grid[y]):
			subProduct = grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3]
			if subProduct > biggestProduct:
				biggestProduct = subProduct
